...
def main():
    ...
    # 2. get password
    # get `name`
    # encrypted = lookup(name)
    # pwd = decrypt(encrypted)
    # display `name` and `pwd`
    ...
    ...
    if not os.path.exists(VAULT_FILE_ADDRESS):
        ...
        for index in range(len(contentLines)):                
            ...
            if contentSplit[0] == name:
                ...
                newContentLine= contentSplit[0]+","+pwd+","+contentSplit[2]+","+newLastUpdatedAt
                updateRow = index
                break
...
def lookup_password_name(name,privateCall=0):
    ...
    '''
    lookUpHashTable = phm(cap=1231,probingMethod="double") # Instantiate a Hash Table using double hashing to store and lookup the name
    
    with open(VAULT_FILE_ADDRESS,'r') as file:
        for line in file:            
            fields = line.rstrip().split(",")
            lookUpHashTable[fields[0]] = fields[1] # fields[0] --> password name // fields[1] --> password
    
    lookupValue = lookUpHashTable[name] # the encrypted password. Use lookUpHashTable[name] to return more data points such as created date and updated date
    ...
    with open(VAULT_FILE_ADDRESS,'r') as file:
        for line in file:     
            ...
            if fields[0] !=   "password_name":                    
                mostRecentUpdatedPasswordNameHeap.add(fields[3],fields[0]) # fields[3] --> Most Recent Updated At || fields[0] --> Password Name
...
def test():
    ...
    lookup_password_name("TemesvariOakton") # Need to find a way to print this accordingly. Currenlty only printing "False"
